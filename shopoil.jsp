...
function cs()
{	
	var s=document.shopoil.items;
	
	
			if(s=="Vegetables")
			{
					document.shopoil.action="index.html";
					document.shopoil.submit();
			}
...
document.shopoil.action="savecart.jsp";
document.shopoil.submit();
}
...
var r=confirm("Do you want to Drop this item from Your CART.?");
	if(r==true)
	{
		...
		document.shopoil.action="delcart.jsp";
		document.shopoil.submit();
	}
//alert("");}
}}
...
function pur()
{
	if(document.shopoil.total.value==0)
	{
		alert("Please Drop any items into Cart to Purchase..!!");
	}
	else
	{
	document.shopoil.action="order.jsp";
	document.shopoil.submit(); }
}
</script>

<form name="shopoil" method="get">
<body>
...
<input type="hidden" name="qt" id="qt">
 ...
 <%
		...
		String connectionURL = "jdbc:mysql://localhost/grocery";
              		String Qry="select *from items where category='oil'";
